...
export async function POST(request: NextRequest) {
  const limited = rateLimit(`webhook:mercadopago:${ipFromHeaders(request.headers)}`, { limit: 60, windowMs: 60_000 });
  if (!limited.success) return NextResponse.json({ error: "rate limited" }, { status: 429 });

  const rawBody = await request.text();
  const { status, body } = await handlePaymentWebhook(getProviderById("MERCADOPAGO"), rawBody, request.headers);
