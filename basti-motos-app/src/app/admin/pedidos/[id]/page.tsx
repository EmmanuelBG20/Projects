...
export default async function AdminPedidoDetailPage({ params }: { params: Params }) {
  ...
  const order = await prisma.order.findUnique({
    where: { id },
    include: { items: true, address: true, payment: true, user: true, coupon: true },
  });
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          ...
          <div className="glass-card p-5">
            <h2 className="mb-3 font-display font-bold text-white">Actualizar estado logístico</h2>
        ...
        <div className="space-y-6">
          <div className="glass-card p-5">
            <h2 className="mb-3 font-display font-bold text-white">Cliente</h2>
          ...
          <div className="glass-card p-5">
            <h2 className="mb-3 font-display font-bold text-white">Dirección</h2>
          ...
          {order.payment && (
            <div className="glass-card p-5">
              ...
              <p className="text-sm text-neutral-400">Proveedor: {order.payment.provider}</p>
              <p className="text-sm text-neutral-400">Referencia: {order.payment.reference}</p>
              <p className="text-sm text-neutral-400">Transacción: {order.payment.transactionId ?? "—"}</p>
